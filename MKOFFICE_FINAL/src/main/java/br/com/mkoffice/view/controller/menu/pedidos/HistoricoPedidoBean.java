...
//		 Map<String, BigDecimal>calcValores = PedidoCalcUtils.calcularValorTotalPedido(listaDePedidosRealizados);
...
//		 vlrTotalAtacadoLbl = calcValores.get("vlrTotalAtacado");
	...
	public String pesquisarFiltro() {
		listaDePedidosRealizados = pedidoBO.consultarPedidos(codPedidoFiltro, dataInicioFiltro, dataFinalFiltro, getLoginBean().getUsuario().getId());
		pedidoSelecionado = null;
...
//	=============================== M�TODOS PRIVATES =============================== 
	private void isDesabilitaPagamentoParcela(){
		for (int i = 0; i < pedidoSelecionado.getParcelas().size(); i++) {
			if(pedidoSelecionado.getParcelas().get(i).isQuitado()){
				pedidoSelecionado.getParcelas().get(i).setDesabilitaPagamentoParcela(true);
			}else{
				if( i > 0){
					if(pedidoSelecionado.getParcelas().get(i-1).isQuitado()){
						if(i == (pedidoSelecionado.getParcelas().size()-1)){
							pedidoSelecionado.getParcelas().get(i).setDesabilitaPagamentoParcela(false);
						}else{
							if(!pedidoSelecionado.getParcelas().get(i-1).isDesabilitaPagamentoParcela()){
								pedidoSelecionado.getParcelas().get(i).setDesabilitaPagamentoParcela(true);
							}else{
								pedidoSelecionado.getParcelas().get(i).setDesabilitaPagamentoParcela(false);
							}
	...
	private void carregarListaPedidosRealizados(){
		listaDePedidosRealizados = pedidoBO.listarPedidosRealizados(getLoginBean().getUsuario().getId());
	}
